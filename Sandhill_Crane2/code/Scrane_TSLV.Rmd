...
ssfdat.crane.final3 <- ssfdat.crane.final2 %>% mutate(tslv = tslv*0.000694444) %>% mutate(tslv = ifelse(tslv < 0, NA, tslv)) %>% na.omit()
ssfdat.crane.final3
...
tslv.visualization <-  ggplot()+
  ...
  geom_point(data = ssfdat.crane.final3 %>% filter(case_ == TRUE), aes(x=x1_, y=y1_, col = tslv), size =0.5)+
  scale_color_viridis(option = "viridis")+
  theme(# Hide panel borders and remove grid lines
  panel.border = element_blank(),
  panel.grid.major = element_blank(),
  panel.grid.minor = element_blank())+
  ggtitle("Across All Years")+
  facet_wrap(~category)
...
# 2020
tslv.visualization20 <-  ggplot()+
  geom_sf(data = memory.map.sf, fill = "white")+
  #geom_path(data = ssfdat.crane.final2, aes(x=x1_, y=y1_, col = tslv))+
  geom_point(data = ssfdat.crane.final3 %>% filter(case_ == TRUE & year == "2020"), aes(x=x1_, y=y1_, col = tslv), size =0.5)+
  scale_color_viridis(option = "viridis")+
  theme(# Hide panel borders and remove grid lines
  panel.border = element_blank(),
  panel.grid.major = element_blank(),
...
tslv.visualization20

# save the image 
ggsave("tslv20.plot.png", tslv.visualization20, width = 12, height = 7, dpi = 700, bg = NA)
```
...
# only tslv predictor 
ssfdat.crane.final3 %>% filter(t1_ > "2016-07-15 05:07:41") %>% amt::fit_issf(case_ ~ nlcd+ tslv+ category:tslv+ log(sl_)+ cos(ta_)+ strata(step_id_)) %>% summary()
```
...
tslv.model <- ssfdat.crane.final3 %>% filter(t1_ > "2016-07-15 05:07:41") %>% amt::fit_issf(case_ ~ nlcd+ tslv+ category:tslv+ log(sl_)+ cos(ta_)+ strata(step_id_))
tslv.model
