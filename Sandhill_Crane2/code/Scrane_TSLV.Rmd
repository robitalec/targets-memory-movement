...
ssfdat.grid.id = point.in.poly(ssfdat.crane.sf.endpoints, memory.map.sf) %>% as.data.frame
...
tslv.visualization <-  ggplot()+
  ...
  #geom_path(data = ssfdat.crane.final1, aes(x=x1_, y=y1_, col = tslv))+
  geom_point(data = ssfdat.crane.final1 %>% filter(case_ == TRUE), aes(x=x1_, y=y1_, col = tslv), size =1.5)+
...
ssfdat.crane.final1 %>% filter(t1_ > "2016-07-15 05:07:41") %>% amt::fit_issf(case_ ~ Land_Cover+ log(sl_)+ cos(ta_)+ strata(step_id_)) %>% summary()

# only tslv predictor 
ssfdat.crane.final1 %>% filter(t1_ > "2016-07-15 05:07:41") %>% amt::fit_issf(case_ ~ Land_Cover+ tslv+ log(sl_)+ cos(ta_)+ strata(step_id_)) %>% summary()
```
save the filtered data
```{r save the filtered data}
#write_csv(ssfdat.crane.final1, path = here("data", "ssfdat.crane.final.tslv.csv"))
```
...
tslv.model <- ssfdat.crane.final1 %>% filter(t1_ > "2016-07-15 05:07:41") %>% amt::fit_issf(case_ ~ Land_Cover+ tslv+ log(sl_)+ cos(ta_)+ strata(step_id_))
tslv.model
