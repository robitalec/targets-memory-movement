...
Write function to calculate run length (v.idx) to specified value (y). If the specified run value (y) is the first value it returns a 0 and if it is missing then a NA is returned. - **current TSLV calculates the number of revisitation per grid** 
```{r tslv}
# tslv function
...
# you can clearly see that there are some missing values although animals definitely have visited the same locations - need to deal with the missing locations 
...
# Observed locations: calculate TSLV 
...
# NAs will be replaced by calculating the difference between the burn-in time - current time of the observed locations
bear.obs.pts <- bear.obs.pts  %>% dplyr::mutate(tslv = ifelse(is.na(tslv), difftime(t,"2005-05-24 00:18:00.00", units = "hours"), tslv))

bear.obs.pts

## add empty values for random locations per used location
...
# set all the random point tslv as 0 first
...
# check the data - now the data must have all the observed location tslv values except those ones that animals first time visiting as NA and all the random points tslv values are 0 
bear.pts.tslv
```

# Random locations: calculate TSLV 

Calculate the TSLV values for random points based on the used points 
```{r calculate the tslv for the random points}
# copy the current data frame 
bear.pts.tslv1 <- bear.pts.tslv
bear.pts.tslv1

# 1st step: TSLV calculation for random points that are within same grid size as the observed location per step 

# a new dataframe 
bear.pts.tslv1 <- bear.pts.tslv1 %>% 
  # slice the data until the first value is the observed location 
  slice(which.max(case == TRUE) : n()) %>% 
  # group by step id and grid id 
  group_by(step_id, grid_id) %>% 
  # if the grid id for random point is equal to the grid id for true point per each step id, set the tslv for the random points same as the observed location 
  mutate(tslv1 = ifelse(grid_id == first(grid_id), across(tslv, first), NA)) %>% 
  # ungroup data
  ungroup %>% 
  # unnest data
  unnest

# 2nd step: TSLV calculation for random points that are different grid size than the observed location per step  

# convert the logical case value (TRUE/FALSE) to numeric 
bear.pts.tslv1$case <- as.numeric(bear.pts.tslv1$case)

# a new dataframe 
bear.pts.tslv2 <- bear.pts.tslv1 %>% 
  # filter only the na values in tslv column and/or true case
  filter(is.na(tslv1) | case == 1) %>%
  arrange(grid_id, step_id)

bear.pts.tslv2

# remember, there are some random locations that do not contain observed locations per step id - need to remove them for the calculation (they are for the burn-in values)
# select the grids that only have random locations but not observed 
no.obs.loc.ran <- bear.pts.tslv2 %>% 
  group_by(grid_id) %>% 
  filter(!any(case == "1"))

no.obs.loc.ran

# remove the no.obs.loc.ran data from the original data 
bear.pts.tslv2 <- bear.pts.tslv2 %>% anti_join(no.obs.loc.ran)

bear.pts.tslv2

# make an index of the latest case 
last_case_index <- cumsum(bear.pts.tslv2$case) + 1

# shift it by one to the right 
last_case_index <- c(1, last_case_index[1:length(last_case_index) - 1])
...
last_case_date <- c(as.POSIXct(NA), bear.pts.tslv2[which(bear.pts.tslv2$case==1), "t"])[last_case_index]

# substract the event's date with the date of the last event
bear.pts.tslv2$tae <- difftime(bear.pts.tslv2$t, last_case_date, units= "hours")

# remove all the random points that were generated before the actual observed points exist
bear.pts.tslv2 <- bear.pts.tslv2 %>% 
  mutate(tae = ifelse(case == 1, NA, tae)) %>% 
  group_by(grid_id) %>% 
  filter(row_number() >= min(which(case == 1))) %>% 
  ungroup

bear.pts.tslv2

# only select the newly calculated the random point tslv values 
diff.ran.tslv.vals <- bear.pts.tslv2 %>% 
  select(x, y, t, case, step_id, grid_id, tae) %>% 
  filter(!is.na(tae))

# left join those diff random tslv values to the original data to bear.pts.tslv1 to complete the entire calculation
bear.pts.tslv.final <- left_join(bear.pts.tslv1, diff.ran.tslv.vals, by = c("x", "y", "t", "step_id", "grid_id")) %>% select(-case.y)

# coalesce two columns together as final.tslv and only select the useful columns 
bear.pts.tslv.final <- bear.pts.tslv.final %>% 
  mutate(final.tslv = coalesce(tslv1, tae)) %>% 
  select(-c(tslv, tslv1, tae)) %>%
  'colnames<-'(c("x","y", "t", "case", "step_id", "grid_id", "tslv")) %>%
  mutate(case = as.logical(case))

bear.pts.tslv.final  
```

# Finalize the data with merging the calculated observed and random tslv 

merge back to the original data 
```{r merge back}
# original ssf data sort by step id 
ssfdat.bear.final <- ssfdat.bear.final[order(ssfdat.bear.final$step_id_),]

ssfdat.bear.final


# add used location tslv

# only select the observed location tslv and change the col names 
obs.pts.tslv <-  bear.pts.tslv.final %>% filter(case == TRUE) %>% 'colnames<-' (c("x1_", "y1_", "t1_", "case_", "step_id_", "grid_id", "obs.tslv"))
obs.pts.tslv

# left join the observed location tslv to the original data 
ssfdat.bear.final <- left_join(ssfdat.bear.final, obs.pts.tslv, by = c("x1_", "y1_", "t1_", "case_","step_id_", "grid_id"))
ssfdat.bear.final

# add random location tslv 

# only select the random location tslv and change the col names 
ran.pts.tslv <- bear.pts.tslv.final %>% filter(case == FALSE) %>% 'colnames<-' (c("x2_", "y2_", "t2_", "case_", "step_id_", "grid_id", "ran.tslv"))
ran.pts.tslv

# left join the random location tslv to the original data 
ssfdat.bear.final <- left_join(ssfdat.bear.final, ran.pts.tslv, by = c("x2_", "y2_", "t2_", "case_","step_id_", "grid_id"))
ssfdat.bear.final

# coalesce the obs and ran tslv columns together and save it as tslv column 
ssfdat.bear.final <- ssfdat.bear.final %>%  mutate(tslv = coalesce(obs.tslv, ran.tslv)) %>% select(-c("obs.tslv", "ran.tslv"))
ssfdat.bear.final
...
#write.csv(ssfdat.bear.final, "ssfdat.bear.final.tslv.csv")
```

# Calculat the burn-in values 

All the NA points are indicating that the animal has never visited the locations before 

filter the burn-in phases and replace NA random points TSLV to current time for the random points subtract the burn-in value: 2005-05-24 00:18:00.00
...
ssfdat.bear.final1 %>% filter(grid_id == 300)
```
