...
bear.pts.tslv1 <- setDT(bear.pts.tslv1)[, ran.loc.tslv.id := ifelse(case != lag(case), diff(t), NA), by = grid_id]
bear.pts.tslv1
...
bear.pts.tslv1 <- bear.pts.tslv1 %>% select(x, y, t, step_id, grid_id, result)
...
bear.pts.tslv4 %>% filter(!is.na(final.tslv))
```
...
# check the data 
ssfdat.bear.final %>% select(t1_,t2_,step_id_, case_, grid_id, tslv)
...
ssfdat.tslv.vis.dat <- bear.pts.tslv4 %>% filter(step_id > 365 & case == TRUE & !is.na(final.tslv)) %>% dplyr::select(x, y, final.tslv) %>% 'colnames<-'(c("x","y", "tslv"))
...
ssfdat.bear.final %>% filter(step_id_ > 365 & !is.na(tslv)) %>% amt::fit_issf(case_ ~ berries+ log(sl_)+ cos(ta_)+ strata(step_id_)) %>% summary()

# TSLV predictor included
ssfdat.bear.final %>% filter(step_id_ > 365 & !is.na(tslv)) %>% amt::fit_issf(case_ ~ berries+ tslv+ log(sl_)+ cos(ta_)+ strata(step_id_)) %>% summary()

# Interaction between TSLV and berries
ssfdat.bear.final %>% filter(step_id_ > 365 & !is.na(tslv)) %>% amt::fit_issf(case_ ~ berries+ tslv+ tslv:berries+ log(sl_)+ cos(ta_)+ strata(step_id_)) %>% summary()
```
