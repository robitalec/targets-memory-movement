...
setDT(bear.pts.tslv1)[, ran.loc.tslv.id := ifelse(case != lag(case), diff(t), NA), by = grid_id]
bear.pts.tslv1
...
ssfdat.tslv.vis.dat <- bear.pts.tslv4 %>% filter(step_id > 365 & case == TRUE) %>% dplyr::select(x, y, final.tslv) %>% 'colnames<-'(c("x","y", "tslv"))
