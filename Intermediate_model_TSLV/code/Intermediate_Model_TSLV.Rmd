...
ssfdat.bear.final %>% filter(year != "2005") %>% amt::fit_issf(case_ ~ berries+ log(sl_)+ cos(ta_)+ strata(step_id_)) %>% summary()
...
ssfdat.bear.final %>% filter(year != "2005" & !is.na(tslv)) %>% amt::fit_issf(case_ ~ berries+ tslv*berries+ log(sl_)+ cos(ta_)) %>% summary()
```
