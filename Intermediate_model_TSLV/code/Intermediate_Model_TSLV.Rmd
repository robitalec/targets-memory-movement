...
nest.ran.buf$current.time.step = c(1:511)
...
obs.tslv.df <- obs.tslv %>% dplyr::select(pt.id, tslv)

# Random - Apply same below 
ran.loc.sf <- ran.loc.sf %>% mutate(pt.id = c(1:534))

ran.tslv.df <- ran.tslv %>% dplyr::select(pt.id, tslv)
```
...
obs.df1 <- obs.df %>% mutate(x = unlist(map(obs.df$geometry,1)),
                   y = unlist(map(obs.df$geometry,2))) %>% dplyr::select(x, y, tslv) %>% st_drop_geometry()

## random
ran.df1 <- ran.df %>% mutate(x = unlist(map(ran.df$geometry,1)),
                   y = unlist(map(ran.df$geometry,2))) %>% dplyr::select(x, y, tslv) %>% st_drop_geometry() 
```
