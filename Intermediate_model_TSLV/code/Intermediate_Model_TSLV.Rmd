...
# If the random locations are in the same grid cells as the used locations per step id, set the same tslv as the observed locations 
ssfdat.bear.final1 <- ssfdat.bear.final1 %>% slice(which.max(case_ == "TRUE") : n()) %>% as.data.frame() %>% group_by(step_id_) %>% mutate(tslv = ifelse(first(grid_id) == grid_id, first(tslv), tslv)) %>% ungroup()
...
# only tslv predictor 
ssfdat.bear.final1 %>% filter(year != 2005) %>% amt::fit_clogit(case_ ~ berries+ tslv+ log(sl_)+ cos(ta_)+ strata(step_id_)) %>% summary()
