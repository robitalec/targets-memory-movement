...
ssfdat.bear.final1 %>% filter(t1_ > "2006-05-24 04:18:00.00") %>% amt::fit_issf(case_ ~ berries+ log_sl_+ cos_ta_+ strata(step_id_)) %>% summary()

# with tslv predictor 
ssfdat.bear.final1 %>% filter(t1_ > "2006-05-24 04:18:00.00") %>% amt::fit_issf(case_ ~ berries + tslv + log_sl_+ cos_ta_ + strata(step_id_)) %>% summary()

# quadratic terms of tslv x berries 
ssfdat.bear.final1 %>% filter(t1_ > "2006-05-24 04:18:00.00") %>% amt::fit_issf(
  case_ ~ berries + tslv + I(tslv^2):berries +log_sl_+ cos_ta_ + strata(step_id_)
) %>% summary()
```
...
tslv.model <- ssfdat.bear.final1 %>% filter(t1_ > "2006-05-24 04:18:00.00") %>% amt::fit_issf(case_ ~ berries + tslv + I(tslv^3) + log_sl_+ cos_ta_ + strata(step_id_), model = TRUE)
...
# plot usinig ggplot2
RSS.TSLV <- ggplot(lr_m1$df, aes(x = tslv_x1, y = log_rss)) +
  geom_line(size = 1) +
  geom_hline(yintercept = 0,
             linetype = "dashed",
             color = "gray30") +
  xlab("Time Since Last Visit (Day)") +
...
```
It looks like the bear selects intermediate TSLV and the non-linear effect is estimated. 
